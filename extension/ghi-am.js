 ...
 *
 * Vì sao lưu vào `chrome.storage.local` chứ không phải IndexedDB
 * -------------------------------------------------------------
 ...
 * `chrome.storage.local` thì cả hai bên nhìn thấy như nhau.
 *
...
(function (goc) {
  ...
  async function docKho() {
    try {
      const r = await chrome.storage.local.get(KHOA);
      return locKho(r[KHOA] || {}, Date.now());
    } catch (e) { return {}; }
  }
  ...
  async function don() {
    ...
    try { await chrome.storage.local.set({ [KHOA]: kho }); } catch (e) { /* đầy thì thôi */ }
    return Object.keys(kho).length;
  }

  async function luu(id, ban) {
    if (!id || !ban || !ban.b64) return false;
    const kho = await docKho();
    kho[id] = { b64: ban.b64, mime: ban.mime || "audio/webm", dai: ban.dai || 0, ts: Date.now() };
    try {
      await chrome.storage.local.set({ [KHOA]: locKho(kho, Date.now()) });
      return true;
  ...
  async function xoa(id) {
    const kho = await docKho();
    if (!(id in kho)) return false;
    delete kho[id];
    try { await chrome.storage.local.set({ [KHOA]: kho }); return true; } catch (e) { return false; }
  }
